...
import signIn from "~services/signin/signin.service";
...
interface ILoginMutation {
  isLoading: boolean;
  mutate: UseMutateFunction<unknown, BaseError, LoginRequest>;
}

const useLogin = (): ILoginMutation => {
  const router = useRouter();

  const { isLoading, mutate } = useMutation<unknown, BaseError, LoginRequest>(signIn, {
    onSuccess: (_data) => {
